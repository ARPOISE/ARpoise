...
#pragma warning restore CS0414
    ...
    public partial class UnityARSessionNativeInterface
    {
        ...
        [DllImport("__Internal")]
        private static extern void StartWorldTrackingSessionWithImages(IntPtr nativeSession, ARKitWorldTrackingSessionConfiguration configuration);

        [DllImport("__Internal")]
        private static extern void StoreTriggerImageInC([MarshalAs(UnmanagedType.LPStr)] string a_name, int a_BufferLength, [MarshalAs(UnmanagedType.LPArray)] byte[] a_ImageBuffer, float a_width);
        ...
        public void StoreTriggerImage(string a_name, int a_BufferLength, byte[] a_ImageBuffer, float a_width)
        {
#if !UNITY_EDITOR
            StoreTriggerImageInC(a_name, a_BufferLength, a_ImageBuffer, a_width);
#else
#endif
        }

        public void RunWithConfigAndImages(ARKitWorldTrackingSessionConfiguration config)
        {
#if !UNITY_EDITOR
            StartWorldTrackingSessionWithImages(m_NativeARSession, config);
#else
            UnityARSessionRunOption runOptions = UnityARSessionRunOption.ARSessionRunOptionRemoveExistingAnchors | UnityARSessionRunOption.ARSessionRunOptionResetTracking;
            CreateRemoteWorldTrackingConnection(config, runOptions);
#endif
