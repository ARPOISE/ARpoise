...
extern "C" void StartWorldTrackingSessionWithImage(
void* nativeSession, ARKitWorldTrackingSessionConfiguration unityConfig,
int a_BufferLength, const void* a_ImageBuffer, float a_width
)
{
    UnityARSession* session = (__bridge UnityARSession*)nativeSession;
    ARWorldTrackingConfiguration* config = [ARWorldTrackingConfiguration new];
    ...
    if(UnityIsARKit_1_5_Supported() && unityConfig.referenceImagesResourceGroup != NULL && strlen(unityConfig.referenceImagesResourceGroup) > 0)
    {
        ...
        NSMutableSet * mSet = [[NSMutableSet alloc] initWithSet:referenceImages];

        ARReferenceImage *image = [[ARReferenceImage alloc] initWithCGImage:cgImage orientation:kCGImagePropertyOrientationUp physicalWidth:a_width];
        image.name = @"dynamicImage";
...
static int maximumNumberOfTriggerImages = 256;
static int numberOfTriggerImages = 0;

static char ** triggerImageNames = NULL;
static int * triggerImageBufferLengths = NULL;
static unsigned char ** triggerImageBuffers = NULL;
static float * triggerImageWidths = NULL;

extern "C" void StoreTriggerImageInC(const char * a_name, int a_BufferLength, const void* a_ImageBuffer, float a_width)
{
    if (numberOfTriggerImages >= maximumNumberOfTriggerImages)
	{
		return;
	}
	...
	if (a_BufferLength > 0)
	{
        if (numberOfTriggerImages == 0)
	    {
			triggerImageNames = (char **)malloc(sizeof(char *) * maximumNumberOfTriggerImages);
			triggerImageBufferLengths = (int *)malloc(sizeof(int) * maximumNumberOfTriggerImages);
		    triggerImageBuffers = (unsigned char **)malloc(sizeof(unsigned char *) * maximumNumberOfTriggerImages);
		    triggerImageWidths = (float *)malloc(sizeof(float) * maximumNumberOfTriggerImages);
	    }

		int length = strlen(a_name) + 1;
	    triggerImageNames[numberOfTriggerImages] = (char *)malloc(length);
	    for (int i = 0; i < length; i++)
		{
		    triggerImageNames[numberOfTriggerImages][i] = a_name[i];
		}
		triggerImageBufferLengths[numberOfTriggerImages] = a_BufferLength;
		triggerImageBuffers[numberOfTriggerImages] = (unsigned char *)malloc(a_BufferLength);
	    for (int i = 0; i < a_BufferLength; i++)
		{
		    triggerImageBuffers[numberOfTriggerImages][i] = ((unsigned char *)a_ImageBuffer)[i];
		}
		triggerImageWidths[numberOfTriggerImages] = a_width;

		numberOfTriggerImages++;
	}
}

extern "C" void StartWorldTrackingSessionWithImages(void* nativeSession, ARKitWorldTrackingSessionConfiguration unityConfig)
{
    if (numberOfTriggerImages < 1)
	{
	    StartWorldTrackingSession(nativeSession, unityConfig);
		return;
	}
    ...
    if(UnityIsARKit_1_5_Supported() && unityConfig.referenceImagesResourceGroup != NULL && strlen(unityConfig.referenceImagesResourceGroup) > 0)
    {
		...
		for (int i = 0; i < numberOfTriggerImages; i++)
		{
            NSData* imageData = [[NSData alloc] initWithBytes:triggerImageBuffers[i] length:triggerImageBufferLengths[i]];
            UIImage* uiimage = [[UIImage alloc] initWithData:imageData];
            CGImageRef cgImage = [uiimage CGImage];

            ARReferenceImage *image = [[ARReferenceImage alloc] initWithCGImage:cgImage orientation:kCGImagePropertyOrientationUp physicalWidth:triggerImageWidths[i]];
            image.name = [[NSString alloc] initWithUTF8String:triggerImageNames[i]];
            [mSet addObject:image];
		}
